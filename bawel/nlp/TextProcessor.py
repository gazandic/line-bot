...
class TextProcessor(object):
    def __init__(self, token_kata):
        ...
        self.listActionInEvent = {"lihat": "lihat", "ganti": "ubah", "buat": "tambah", "tambah": "tambah",
        ...
        self.listActionInPengeluaran = {"lihat": "lihat", "ganti": "ubah", "buat": "tambah", "tambah": "tambah",
                                        "bikin": "tambah", "ubah": "ubah", "hapus": "hapus", "lapor": "report",
                                        "liat": "lihat", "report": "report", "eval": "report"}
        self.listErrorInEvent = {"lihat": errorLiatJadwal, "ikut": errorIkutJadwal, "hapus": errorHapusJadwal,
    ...
    def isCalled(self, sentence):
        checkifCalled = re.compile(
            r'({0})'.format(
                self.keyWordToCall),
            flags=re.IGNORECASE)
        return checkifCalled.findall(sentence)
    ...
    def checkPerson(self, sentence):
        entities = self.kataClient.getEntities(sentence)
        return list(
                            ...
                            for eventkey in self.keyWordToEvent:
                                for actiondetection in self.listActionDetection:
                                    ...
                                    pengeluaran_nameRe = re.compile(
                                        r'{0}\s+(.+)\s+{1}'.format(pengeluaranKey, kataKunciEvent))

                                    event_nameRe = re.compile(r'{0}\s+(.+)\s+{1}'.format(kataKunciEvent, "sebesar"),
                                                              flags=re.IGNORECASE)
                            ...
                            try:
                                for eventkey in self.keyWordToEvent:
                                    for actiondetection in self.listActionDetection:
                                        ...
                                        event_nameRe = re.compile(r'{0}\s+(.+)\s+{1}'.format(kataKunciEvent, "oleh"),
                                                                  flags=re.IGNORECASE)
                                        if (event_nameRe.findall(sentence) and pengeluaran_nameRe.findall(sentence)):
                                            event_name = event_nameRe.findall(sentence)[0]
                                            pengeluaran_name = pengeluaran_nameRe.findall(sentence)[0]
                ...
                try:
                    if action in ["buat", "tambah", "bikin", "ubah", "ganti"]:
                        ...
                        self.jsonToSend = {'type': 'jadwal', 'command': self.listActionInEvent[action],
                                           'data': {'date': datetypedate, 'event_name': event_name}}
                        break
                        ...
                        if persons:
                            print(sentence)
                            try:
                                event_nameRe = re.compile(r'{0}\s+(.*)\s+{1}'.format(eventKey, "oleh"),
                                                          flags=re.IGNORECASE)
                                event_name = event_nameRe.findall(sentence)[0]
                            except:
    ...
    def dateParser(self, dateSentence, timeSentence):
        ...
        yearDetected = False
        now = datetime.now()
        monthNameToNumber = {"januari": 1, "februari": 2, "maret": 3, "april": 4, "mei": 5, "juni": 6, "juli": 7,
        ...
        if (timeSentence):
            ...
            if s[len(s) - 1] == "malam" and digit <= 12 and digit >= 6 or s[
                        len(s) - 1] in keteranganWaktu and digit <= 7:
                digit = digit + 12
                if digit >= 24:
                    digit -= 24
                s[0] = str(digit)
    ...
    def getEntities(self, text):
        return self.kataClient.getEntities(text)
