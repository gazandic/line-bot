...
class TextProcessor(object):
    def __init__(self):
        ...
        self.listActionInEvent = {
            "lihat": "lihat",
            "ganti": "ubah",
            "buat": "tambah",
            "tambah": "tambah",
            "bikin": "tambah",
            "gajadi ikut": "gajadiikut",
            "ga jadi ikut": "gajadiikut",
            "gak jadi ikut": "gajadiikut",
            "gak ikut": "gajadiikut",
            "ikut": "ikut",
            "ubah": "ubah",
            "hapus": "hapus",
    ...
    def checkActionPengeluaran(self, sentence, pengeluaranKey):
        for action in self.listActionInPengeluaran:
            isContain = re.compile(
                r'\b({0})\b'.format(action),
                flags=re.IGNORECASE)
            persons = self.checkPerson(sentence)
            if(isContain.findall(sentence)):
                if action in ["buat", "tambah", "bikin"]:
                    if(self.checkAmount(sentence) is not None):
                        amount = self.checkAmount(sentence)
                        pengeluaran_nameRe = re.compile(
                            r'{0}\s+(.*)\s+{1}'.format(pengeluaranKey, "sebesar"), flags=re.IGNORECASE)
                        pengeluaran_name = 'unknown'
                        if (pengeluaran_nameRe.findall(sentence)):
                            pengeluaran_name = pengeluaran_nameRe.findall(sentence)[
                                0]
                        if persons:
                            self.jsonToSend = {'type': 'pengeluaran', 'command': action, 'data': {
                                'amount': amount, 'event_name': pengeluaran_name, 'persons': persons}}
                        else:
                            self.jsonToSend = {
                                'type': 'pengeluaran',
                                'error': ['no persons'],
                                ...
                                'data': {
                                    'amount': amount,
                                    'event_name': pengeluaran_name}}
                        break
                    else:
                        if persons:
                            pengeluaran_nameRe = re.compile(
                                r'{0}\s+(.*)\s+{1}'.format(pengeluaranKey, "oleh"), flags=re.IGNORECASE)
                            pengeluaran_name = pengeluaran_nameRe.findall(sentence)[
                                0]
                            self.jsonToSend = {
                                'type': 'pengeluaran',
                            ...
                            pengeluaran_nameRe = re.compile(
                                r'{0}\s+(.*)\s+{1}'.format(pengeluaranKey), flags=re.IGNORECASE)
                            pengeluaran_name = pengeluaran_nameRe.findall(sentence)[
                                0]
                            self.jsonToSend = {
                                'type': 'pengeluaran', 'error': [
                                    'no persons', 'no amount'], 'command': action, 'data': {
                                    'amount': 0, 'event_name': pengeluaran_name}}
                    ...
                    if amount is not None:
                        ...
                        if (pengeluaran_nameRe.findall(sentence)):
                            pengeluaran_name = pengeluaran_nameRe.findall(sentence)[
                                0]
                            self.jsonToSend = {
                                'type': 'pengeluaran', 'command': action, 'data': {
                                    'amount': amount, 'event_name': pengeluaran_name}}
                        else:
                            self.jsonToSend = {
                                'type': 'pengeluaran',
                                'command': action,
                        ...
                        if (pengeluaran_nameRe.findall(sentence)):
                            ...
                            self.jsonToSend = {
                                'type': 'pengeluaran', 'command': action, 'data': {
                                    'persons': person, 'event_name': pengeluaran_name}}
                        else:
                            self.jsonToSend = {
                                'type': 'pengeluaran',
                                'command': action,
                    ...
                    pengeluaran_nameRe = re.compile(
                        r'{0}\s(.+)'.format(pengeluaranKey))
                    pengeluaran_name = pengeluaran_nameRe.findall(sentence)[0]
                    self.jsonToSend = {
                        'type': u'Pengeluaran', 'command': action, 'data': {
                            'event_name': pengeluaran_name}}
    ...
    def checkActionEvent(self, sentence, eventKey):
        ...
        for key in keyTime:
            ...
            if(timeFound):
                ...
                excludeTimeSentence = re.compile(
                    r'{0}'.format(timeSentenceToExclude))
            ...
            if(isContain.findall(temp_sentence)):
                try:
                    if action in ["buat", "tambah", "bikin", "ubah", "ganti"]:
                        event_nameRe = re.compile(
                            r'{0}\s(.+)\s{1}'.format(eventKey, "tanggal"), flags=re.IGNORECASE)
                        event_name = event_nameRe.findall(sentence)[0]
                        self.jsonToSend = {
                            'type': 'jadwal',
                            'command': self.listActionInEvent[action],
                            'data': {
                                'date': self.json_serial(datetypedate),
                        ...
                        if action in [
                            ...
                            if persons:
                                ...
                                try:
                                    event_nameRe = re.compile(
                                        r'{0}\s+(.*)\s+{1}'.format(eventKey, "oleh"), flags=re.IGNORECASE)
                                    event_name = event_nameRe.findall(sentence)[
                                        0]
                                except BaseException:
                                    event_nameRe = re.compile(
                                        r'{0}\s(.+)\s'.format(eventKey), flags=re.IGNORECASE)
                                    event_name = event_nameRe.findall(sentence)[
                                        0]
                                self.jsonToSend = {
                                    'type': 'jadwal', 'command': self.listActionInEvent[action], 'data': {
                                        'event_name': event_name, 'persons': persons}}
                                break
                            else:
                                event_nameRe = re.compile(
                                    r'{0}\s(.+)\s'.format(eventKey), flags=re.IGNORECASE)
                            ...
                            try:
                                event_nameRe = re.compile(
                                    r'{0}\s(.+)'.format(eventKey), flags=re.IGNORECASE)
                                event_name = event_nameRe.findall(sentence)[0]
                                self.jsonToSend = {
                                    'type': 'jadwal',
                                    'command': self.listActionInEvent[action],
    ...
    def dateParser(self, dateSentence, timeSentence):
        ...
        for monthname in monthNameToNumber:
            if (re.compile(r'\b({0})\b'.format(
                    monthname), flags=re.IGNORECASE)):
                bulanDetected = True
                dateSentence = re.sub(
                    monthname, str(
                        monthNameToNumber[monthname]), dateSentence, flags=re.IGNORECASE)
        ...
        if (timeSentence):
            ...
            digit = int(s[0])
            if s[len(s) -
                 1] == "malam" and digit <= 12 and digit >= 6 or s[len(s) -
                                                                   1] in keteranganWaktu and digit <= 7:
                digit = digit + 12
                if digit >= 24:
                    digit -= 24
                s[0] = str(digit)
            ...
            if len(timeDigits) == 1:
                ...
                temp_timeSentence = timeDigits[0] + \
